...
<div>
        ...
        async defer></script>   
          ...
          <audio autoplay>
            <source src="{{url('/')}}/public/sound/on.mp3" type="audio/ogg">  
          </audio>
          <h4>From: <?php if($data['singlemeaasge']->receiver_type=="admin"){echo 'Life Cycle'; } ?></h4>
          <h4>Message Published: {{$data['singlemeaasge']->created_at}}</h4>
          <hr>
          <p align="justify">{{$data['singlemeaasge']->message}}</p>
          <hr>
          <a class="btn-xs btn-primary" href="{{url('/')}}/singlemeaasge/agree/{{$data['singlemeaasge']->id}}">Response for Donation</a>
          <a class="btn-xs btn-danger" href="{{url('/')}}/singlemeaasge/disagree/{{$data['singlemeaasge']->id}}">Unavailable</a> 
